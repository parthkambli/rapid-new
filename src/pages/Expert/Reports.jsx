...
const AdvocateReports = () => {
  ...
  const fetchReports = async () => {
    try {
      ...
      const response = await apiClient.get(apiEndpoints.reports.expertSpecific, { params });
  ...
  const exportToExcel = () => {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Expert Reports');

      // Export the file
      XLSX.writeFile(wb, `expert-reports-${new Date().toISOString().split('T')[0]}.xlsx`);
  ...
  const exportToCSV = () => {
    try {
      ...
      link.setAttribute('download', `Expert-reports-${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const printReport = () => {
    ...
    const reportHTML = `
      <html>
        ...
        <body>
          <h1>Expert Reports - ${new Date().toLocaleDateString()}</h1>
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="bg-white rounded-lg shadow-md p-6 mb-6">
        <h1 className="text-2xl font-bold text-gray-800 mb-2">Expert Reports</h1>
        <p className="text-gray-600">View and analyze Expert performance and statistics</p>
      </div>
      ...
      <div className="bg-white rounded-lg shadow-md p-6">
        <div className="flex justify-between items-center mb-4">
          <h2 className="text-xl font-semibold text-gray-800">Expert Report Details</h2>

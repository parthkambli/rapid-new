    ...
    try {
      ...
      if (unlinkedSpouseId) {
        ...
        if (mainDoctorId) {
          ...
          const updateMainDoctorPromise = apiClient.put(apiEndpoints.doctors.updateAdditional(mainDoctorId), {
          ...
          const unlinkSpousePromise = apiClient.put(apiEndpoints.doctors.updateAdditional(unlinkedSpouseId), {
            linkedDoctorId: null,
            relationshipType: null,
          });
      ...
      if (mainDoctor.linkedDoctorId && mainDoctor.relationshipType === 'spouse' && !formData.linkAsSpouses) {
        // Unlink main doctor
        await apiClient.put(apiEndpoints.doctors.updateAdditional(mainDoctor._id), {
          linkedDoctorId: null,
          relationshipType: null
        });
        // Unlink spouse doctor if its ID is known
        if (mainDoctor.linkedDoctorId) {
          await apiClient.put(apiEndpoints.doctors.updateAdditional(mainDoctor.linkedDoctorId), {
            linkedDoctorId: null,
            relationshipType: null
          });
        }
        ...
        await apiClient.put(apiEndpoints.doctors.updateAdditional(mainDoctor._id), mainDoctorUpdateData);
      ...
      <div className="bg-white rounded-lg shadow-lg p-6">
        ...
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex gap-2">
            <button
              onClick={() => navigate(`/sales/view-doctor/${id}`)}
                  ...
                  {formData.documents.otherDocs.map((doc, idx) => (
                    <div key={idx} className="flex items-center justify-between p-2 bg-gray-50 rounded">
                      ...
                      <div className="flex space-x-2">
                        <button
                          type="button"
                          onClick={() => window.open(`${process.env.REACT_APP_API_URI || 'https://rapidapis.stellarabodes.in'}/uploads/${doc.split('/').pop()}`, '_blank')}

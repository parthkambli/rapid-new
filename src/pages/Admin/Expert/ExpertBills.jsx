...
const ExpertBills = () => {
  ...
  const fetchBills = async (params = {}) => {
    try {
      ...
      applyFilters(fetchedBills, params);
    } catch (err) {
  ...
  const applyFilters = (bills, params = {}) => {
    let filtered = [...bills];
    ...
    if (params.search) {
      ...
      filtered = filtered.filter(bill =>
        ...
        (bill.expert?.fullName && bill.expert.fullName.toLowerCase().includes(term)) ||
        (bill.expert?.name && bill.expert.name.toLowerCase().includes(term)) ||
        (bill.doctor?.fullName && bill.doctor.fullName.toLowerCase().includes(term))
      );
    }

    // Apply expert filter (client-side text search)
    if (params.expert) {
      const expertTerm = params.expert.toLowerCase();
      filtered = filtered.filter(bill =>
        (bill.expert?.fullName && bill.expert.fullName.toLowerCase().includes(expertTerm)) ||
        (bill.expert?.name && bill.expert.name.toLowerCase().includes(expertTerm))
      );
    }

    // Apply doctor filter (client-side text search)
    if (params.doctor) {
      const doctorTerm = params.doctor.toLowerCase();
      filtered = filtered.filter(bill =>
        (bill.doctor?.fullName && bill.doctor.fullName.toLowerCase().includes(doctorTerm))
      );
    }

    // Apply caseNo filter
    if (params.caseNo) {
      const caseNoTerm = params.caseNo.toLowerCase();
      filtered = filtered.filter(bill =>
        bill.caseNo && bill.caseNo.toLowerCase().includes(caseNoTerm)
      );
    }

    // Apply caseType filter
    if (params.caseType) {
      filtered = filtered.filter(bill =>
        bill.caseType === params.caseType
      );
    }
  ...
  useEffect(() => {
    fetchBills(); // Fetch all bills initially
    fetchFilterData();
  }, []);
    ...
    <div className="max-w-[79vw] mx-auto p-6 bg-white border border-gray-300 rounded-lg shadow-md">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-6 gap-3 mb-6">
        ...
        <input
          ...
          onChange={(e) => handleFilterChange('expert', e.target.value)}
          className="p-2 border border-gray-300 rounded-md bg-gray-100 text-sm focus:outline-none focus:ring-2 focus:ring-[#398C89]"
        />

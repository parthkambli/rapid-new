...
// // View All Tasks Modal Open
// // const openViewTasksModal = async (employee) => {
// //   try {
// //     const res = await apiClient.get(apiEndpoints.salesmanTargets.getTasks(employee._id));
// //     if (res.data.success) {
// //       setSelectedEmployee(employee);
// //       setSelectedTask(res.data.tasks); // array of tasks
// //       setIsEditMode(false);
// //       setModalOpen(true);
// //     }
// //   } catch (err) {
...
// const openViewTasksModal = async (employee) => {
//   try {
//     // Ab tasks already frontend mein hai → direct use kar sakte hai!
//     if (employee.tasks && employee.tasks.length > 0) {
//       setSelectedEmployee(employee);
//       setSelectedTask(employee.tasks); // ← direct pass kar do
//       setIsEditMode(false);
//       setModalOpen(true);
//     } else {
...
//   try {
//     setLoading(true);
//     const response = await apiClient.get('/salesman-targets/employee-all/current', {
...
//     const todayKey = new Date().toISOString().split('T')[0]; // "2025-12-04"

//     // Console debug - yeh bata dega exact structure
//     console.log("RAW API RESPONSE:", response.data);
//     console.log("First item's target:", rawData[0]?.target);
//     console.log("First item's daily field:", rawData[0]?.target?.daily);
...
//       if (target.daily) {
//         // Structure 1: { "2025-12-04": { target: 15, ... } }
...
//       console.log(`Employee ${emp.fullName}: Daily Target = ${dailyTarget}`);

//       // Return EXACT same structure as your old UI
//       return {
//         _id: emp._id,
...
//         // tasks: item.target?.tasks || [], // YE ADD KAR DIYA
//         tasks: target.tasks || [],
//         // Other fields
//         // tasks: [],
...
//     // Check if daily targets are showing
//     if (rows.length > 0) {
//       const firstRow = rows[0];
//       console.log("First row daily target:", firstRow.dailyTarget);
//       console.log("Should be 15");
//     }

//     toast.success(`Loaded ${rows.length} salesmen`);
...
//         params: { q: searchName || searchEmpId, role: 'salesman' }
//       });
//       if (res.data.success) {
...
//           fullName: s.fullName,
//           totalDoctors: s.performanceMetrics?.totalDoctorsAdded || 0,
//           hotLeads: s.performanceMetrics?.totalHotLeads || 0,
//           closeLeads: s.performanceMetrics?.totalCloseLeads || 0,
//           tasks: [],
//           isEditing: false,
//           yearlyTarget: 0, monthlyTarget: 0, dailyTarget: 0,
//           dailyCompleted: 0, monthlyCompleted: 0, yearlyCompleted: 0, totalCompleted: 0,
//         }));
//         setData(results);
//       }
//     } catch (err) {
...
//   const handleEdit = (i) => {
//     setData(prev => prev.map((d, idx) => ({ ...d, isEditing: idx === i })));
//     setEditingRow(i);
//   };
...
//   //     dailyTargets: row.dailyTarget > 0 ? [{ date: todayKey, target: row.dailyTarget }] : []
//   //   };
//   //   await apiClient.post(apiEndpoints.salesmanTargets.createUpdate, payload);
//   // };
...
//   const res = await apiClient.post('/salesman-targets/', payload); // <-- Tera correct endpoint
...
//     //     </div>
//     //   )},
//     // { header: 'Tasks', render: (r,i) => (
//     //     <div className="flex gap-2">
//     //       <button onClick={()=>{setSelectedEmployee({...r,index:i});setIsEditMode(true);setModalOpen(true);}}
//     //         className="w-8 h-8 bg-blue-600 text-white rounded flex-center hover:bg-blue-700">+</button>
//     //       {r.tasks?.length > 0 && (
//     //         <button onClick={()=>{setSelectedEmployee({...r,index:i});setSelectedTask(r.tasks);setModalOpen(true);}}
//     //           className="w-8 h-8 bg-green-600 text-white rounded relative hover:bg-green-700">
//     //           View
//     //           {r.tasks.length > 1 && <span className="absolute -top-1 -right-1 bg-red-500 text-white text-xs rounded-full w-4 h-4">{r.tasks.length}</span>}
//     //         </button>
//     //       )}
//     //     </div>
//     //   )},
...
//       ) : (
//         <div className="flex gap-2">
//           <button onClick={()=>handleEdit(i)} className="w-8 h-8 bg-blue-600 text-white rounded flex-center hover:bg-blue-700">Edit</button>
...
//             columnOrder={['employeeId','fullName','Daily Target','Monthly Target','Yearly Target','Total Stats','Tasks','Actions']}
//             pagination={true}
//             defaultPageSize={10}
//           />
//           <div className="mt-6 flex justify-end gap-4">
//             <button onClick={handleSaveAll} disabled={updating || !data.some(d=>d.isEditing)}
//               className="px-8 py-3 bg-green-600 text-white rounded-lg font-bold disabled:opacity-50">
//               {updating ? 'Saving All...' : 'Save All Changes'}
//             </button>
//           </div>
//         </>
//       )}

//       <TaskModal isOpen={modalOpen} onClose={()=>{setModalOpen(false); fetchData();}}
//         employee={selectedEmployee} task={selectedTask} isEditMode={isEditMode} />
//     </div>
//   );
// };
...
//             data={data}
//             extraColumns={extraColumns}
//             columnOrder={['employeeId','fullName','Daily Target','Monthly Target','Yearly Target','Tasks','Actions','totalDoctors','hotLeads','closeLeads','dailyAchieved']}
//             pagination={true}
//             excludeColumns={['tasks','dailyTarget','monthlyTarget','yearlyTarget']} 
//             defaultPageSize={10}
//           />

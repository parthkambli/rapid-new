...
//       currentAddress: "", currentPin: "", currentCity: "", currentDistrict: "", currentState: "", currentCountry: "",
...
//       address: "", pin: "", city: "", district: "", state: "", country: "",
//       sameAsAbove: false
//     },
...
//             currentCountry: emp.contactDetails?.currentAddress?.country || "India", // Default to India
...
//             permanentCountry: emp.contactDetails?.permanentAddress?.country || "India", // Default to India
...
//             country: emp.familyDetails?.address?.country || "India", // Default to India
...
//           },
//           documents: {
//             // Populate document file paths if they exist
...
//         setFormData(prev => ({
//           ...prev,
//           documents: { ...prev.documents, [field]: { ...prev.documents[field], file: files[0] } }
//         }));
//       } else {
...
//     // Only force uppercase on specific fields that should be uppercase, not all non-email fields
//     if (type !== "select-one" && type !== "radio" && type !== "date" && type !== "number" && !name.includes(".email")) {
//       // Apply uppercase only to text fields that should be uppercase
//       const fieldsToUppercase = ["personal.fullName", "personal.aadhar", "personal.pan", "contact.currentAddress", "contact.permanentAddress", "family.address", "family.fullName", "family.relation"];
//       if (fieldsToUppercase.some(field => name.includes(field))) {
//         finalValue = value.toUpperCase();
//       } else {
//         finalValue = value; // Keep original case
//       }
//     } else {
//       finalValue = value; // Keep original case for email, selects, radios, dates, numbers
//     }
...
//     // Only require current district if it's not the same as permanent address
//     if (!formData.contact.sameAsAbove && !formData.contact.currentDistrict) {
...
//     // Validate health/PA policy metadata if new files are being uploaded
//     if (formData.documents.healthPolicy.file instanceof File &&
...
//     if (formData.documents.paPolicy.file instanceof File &&
...
//       // Only add new files, not existing filenames
...
//       await apiClient.put(apiEndpoints.employees.update(id), formDataToSend, {
...
//               <input name="personal.fullName" value={formData.personal.fullName} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" required />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
...
//                   <input type="radio" name="personal.gender" value="Male" checked={formData.personal.gender === "MALE"} onChange={handleChange} className="mr-2 text-teal-600" required /> Male
//                 </label>
//                 <label className="flex items-center text-sm text-black">
//                   <input type="radio" name="personal.gender" value="Female" checked={formData.personal.gender === "FEMALE"} onChange={handleChange} className="mr-2 text-teal-600" required /> Female
//                 </label>
//               </div>
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-32 text-sm font-medium mb-1 sm:mb-0 text-[#000000]">Aadhar Card Number:</label>
//               <input name="personal.aadhar" value={formData.personal.aadhar} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" required />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-20 text-sm font-medium mb-1 sm:mb-0 text-[#000000]">PAN:</label>
//               <input name="personal.pan" value={formData.personal.pan} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" required />
//             </div>
//           </div>
//         </div>
...
//               <input name="contact.phone" value={formData.contact.phone} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" required />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-20 text-sm mb-1 sm:mb-0 text-[#000000]">WhatsApp :</label>
//               <input name="contact.whatsapp" value={formData.contact.whatsapp} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" />
//             </div>
...
//               <input name="contact.email" value={formData.contact.email} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" required />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center col-span-1 sm:col-span-2">
//               <label className="w-full sm:w-32 text-sm mb-1 sm:mb-0 text-[#000000]">Current Address:</label>
//               <input name="contact.currentAddress" value={formData.contact.currentAddress} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" required />
//               <label className="w-full sm:w-20 text-sm ml-0 sm:ml-4 mt-2 sm:mt-0 mb-1 sm:mb-0 text-[#000000]">Pin code:</label>
//               <input name="contact.currentPin" value={formData.contact.currentPin} onChange={handleChange} className="border border-gray-300 p-2 w-full sm:w-24 text-sm rounded-md focus:ring-2 focus:ring-teal-500" required />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
...
//               <select name="contact.currentCountry" value={formData.contact.currentCountry} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" required>
...
//                 <input type="checkbox" name="contact.sameAsAbove" checked={formData.contact.sameAsAbove} onChange={handleChange} className="mr-2 text-teal-600" /> Same as above
//               </label>
//               <label className="w-full sm:w-20 text-sm ml-0 sm:ml-8 mt-2 sm:mt-0 mb-1 sm:mb-0 text-[#000000]">Pin code:</label>
//               <input name="contact.permanentPin" value={formData.contact.permanentPin} onChange={handleChange} className="border border-gray-300 p-2 w-full sm:w-24 text-sm rounded-md focus:ring-2 focus:ring-teal-500" disabled={formData.contact.sameAsAbove} />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-12 text-sm mb-1 sm:mb-0 text-[#000000]">City :</label>
//               <LocationInput
...
//                 className="w-full"
//                 disabled={formData.contact.sameAsAbove}
//                 required={!formData.contact.sameAsAbove}
//               />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-12 text-sm mb-1 sm:mb-0 text-[#000000]">State:</label>
...
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-16 text-sm mb-1 sm:mb-0 text-[#000000]">Country:</label>
//               <select name="contact.permanentCountry" value={formData.contact.permanentCountry} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" disabled={formData.contact.sameAsAbove}>
//                 <option value="">Select Country</option>
//                 <option value="India">India</option>
//               </select>
//             </div>
...
//               <input name="family.fullName" value={formData.family.fullName} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-32 text-sm mb-1 sm:mb-0 text-[#000000]">Relation With Employee:</label>
//               <input name="family.relation" value={formData.family.relation} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="sm:w-25 text-sm mb-1 md:mr-8 sm:mb-0 text-[#000000]">Phone No :</label>
//               <input name="family.phone" value={formData.family.phone} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-16 text-sm mb-1 sm:mb-0 text-[#000000]">Email:</label>
//               <input name="family.email" value={formData.family.email} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center col-span-1 sm:col-span-2">
//               <label className="w-full sm:w-16 text-sm mb-1 sm:mb-0 text-[#000000]">Address:</label>
//               <label className="flex items-center text-sm ml-0 sm:ml-4 mb-1 sm:mb-0 text-[#000000]">
//                 <input type="checkbox" name="family.sameAsAbove" checked={formData.family.sameAsAbove} onChange={handleChange} className="mr-2 text-teal-600" /> Same as above
//               </label>
//               <label className="w-full sm:w-20 text-sm ml-0 sm:ml-8 mt-2 sm:mt-0 mb-1 sm:mb-0 text-[#000000]">Pin code:</label>
//               <input name="family.pin" value={formData.family.pin} onChange={handleChange} className="border border-gray-300 p-2 w-full sm:w-24 text-sm rounded-md focus:ring-2 focus:ring-teal-500" disabled={formData.family.sameAsAbove} />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-12 text-sm mb-1 sm:mb-0 text-[#000000]">City :</label>
//               <LocationInput
...
//                 className="w-full"
//                 disabled={formData.family.sameAsAbove}
//                 required={!formData.family.sameAsAbove}
//               />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-12 text-sm mb-1 sm:mb-0 text-[#000000]">State:</label>
...
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-16 text-sm mb-1 sm:mb-0 text-[#000000]">Country:</label>
//               <select name="family.country" value={formData.family.country} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" disabled={formData.family.sameAsAbove}>
//                 <option value="">Select Country</option>
//                 <option value="India">India</option>
//               </select>
//             </div>
...
//               <input type="date" name="job.doj" value={formData.job.doj} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" required />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-20 text-sm mb-1 sm:mb-0 text-[#000000]">Role:</label>
//               <select name="job.role" value={formData.job.role} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" required>
...
//               <input name="job.department" value={formData.job.department} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-24 text-sm mb-1 sm:mb-0 text-[#000000]">Designation:</label>
//               <input name="job.designation" value={formData.job.designation} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" required />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-24 text-sm mb-1 sm:mb-0 text-[#000000]">Employee Type:</label>
//               <select name="job.empType" value={formData.job.empType} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" required>
...
//               <input name="job.manager" value={formData.job.manager} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" required />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-24 text-sm mb-1 sm:mb-0 text-[#000000]">Work Location:</label>
//               <input name="job.workLocation" value={formData.job.workLocation} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" required />
//             </div>
//           </div>
//         </div>
...
//               <input type="number" name="salary.basicSalary" value={formData.salary.basicSalary} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-32 text-sm mb-1 sm:mb-0 text-[#000000]">Travelling Allowance :</label>
//               <input type="number" name="salary.travellingAllowance" value={formData.salary.travellingAllowance} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-24 text-sm mb-1 sm:mb-0 text-[#000000]">Daily Allowance :</label>
//               <input type="number" name="salary.dailyAllowance" value={formData.salary.dailyAllowance} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-24 text-sm mb-1 sm:mb-0 text-[#000000]">Mobile Expenses :</label>
//               <input type="number" name="salary.mobileExpenses" value={formData.salary.mobileExpenses} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-24 text-sm mb-1 sm:mb-0 text-[#000000]">Total Salary :</label>
//               <input type="number" name="salary.totalSalary" value={formData.salary.totalSalary} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-32 text-sm mb-1 sm:mb-0 text-[#000000]">Incentive Details :</label>
//               <input name="salary.incentiveDetails" value={formData.salary.incentiveDetails} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" />
//             </div>
//           </div>
//         </div>
...
//               <input name="bank.name" value={formData.bank.name} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" required />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-24 text-sm mb-1 sm:mb-0 text-[#000000]">Bank Name :</label>
//               <input name="bank.bankName" value={formData.bank.bankName} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" required />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-16 text-sm mb-1 sm:mb-0 text-[#000000]">Branch :</label>
//               <input name="bank.branch" value={formData.bank.branch} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" required />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-28 text-sm mb-1 sm:mb-0 text-[#000000]">Account Number :</label>
//               <input name="bank.account" value={formData.bank.account} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" required />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-12 text-sm mb-1 sm:mb-0 text-[#000000]">IFSC :</label>
//               <select name="bank.ifsc" value={formData.bank.ifsc} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" required>
...
//               <input name="education.qualification" value={formData.education.qualification} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-24 text-sm mb-1 sm:mb-0 text-[#000000]">Experience :</label>
//               <input name="education.experience" value={formData.education.experience} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" />
//             </div>
//             <div className="flex flex-col sm:flex-row items-start sm:items-center">
//               <label className="w-full sm:w-24 text-sm mb-1 sm:mb-0 text-[#000000]">Skills :</label>
//               <input name="education.skills" value={formData.education.skills} onChange={handleChange} className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500" />
//             </div>
//           </div>
//         </div>

//         {/* UPLOAD DOCUMENTS */}
//         <h3 className="text-lg font-semibold mb-4 text-[#000000]">Upload Documents (Leave blank to keep existing):</h3>
//         <div className="border border-gray-300 mb-6 p-4 sm:p-6 rounded-lg">
//           <div className="space-y-4">
//             {[
...
//                 <button type="button" onClick={() => fileRefs[doc.key].current?.click()} className="border border-gray-300 bg-gray-100 px-4 py-2 text-sm rounded-md hover:bg-gray-200 w-full sm:w-auto transition-colors">
//                   Choose File
//                 </button>
//                 <input ref={fileRefs[doc.key]} type="file" name={`documents.${doc.key}.file`} onChange={handleChange} className="hidden" />

//                 {/* Show existing file name and preview button if available */}
//                 <div className="flex flex-col sm:flex-row items-start sm:items-center gap-2 w-full sm:w-auto">
//                   <span className="text-sm text-[#000000] truncate max-w-[150px]">
//                     {(() => {
//                       const file = formData.documents[doc.key];
//                       if (doc.key === "paPolicy" || doc.key === "healthPolicy") {
//                         // Check if it's a new uploaded file (File object) or existing filename
...
//                       <>
//                         <input name={`documents.${doc.key}.policyNumber`} value={formData.documents[doc.key].policyNumber} onChange={handleChange} placeholder="Policy Number" className="border p-2 text-sm rounded-md" required />
//                         <input type="date" name={`documents.${doc.key}.startDate`} value={formData.documents[doc.key].startDate} onChange={handleChange} className="border p-2 text-sm rounded-md" required />
//                         <input type="date" name={`documents.${doc.key}.endDate`} value={formData.documents[doc.key].endDate} onChange={handleChange} className="border p-2 text-sm rounded-md" required />
//                       </>
//                     )}
...
//                           ? 'Upload new file to enter health policy details'
//                           : 'Upload new file to enter PA policy details'}
//                       </div>
//                     )}
//                   </div>
//                 )}
...
//           <button type="submit" disabled={saving} className="bg-green-600 text-white px-6 py-2 rounded-md text-sm font-medium hover:bg-green-700 disabled:opacity-50 w-full sm:w-auto">
//             {saving ? "Updating..." : "Update"}
//           </button>
//           <button type="button" onClick={() => navigate(-1)} className="bg-red-600 text-white px-6 py-2 rounded-md text-sm font-medium hover:bg-red-700 w-full sm:w-auto">
//             Cancel
//           </button>
//         </div>
//       </form>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-x-4 sm:gap-x-8 gap-y-4">
            ...
            <div className="flex flex-col sm:flex-row items-start sm:items-center">
              ...
              <select 
                ...
                onChange={handleChange} 
                className="border border-gray-300 p-2 flex-1 text-sm w-full rounded-md focus:ring-2 focus:ring-teal-500 cursor-not-allowed" 
                required
                disabled
              >
                <option value="">Select Role</option>

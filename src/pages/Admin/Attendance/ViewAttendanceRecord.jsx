...
const ViewAttendanceRecord = () => {
  ...
  const fetchAttendanceData = async () => {
    try {
      ...
      // Use the new endpoint that includes all dates (even missing ones)
      const response = await apiClient.get(apiEndpoints.attendance.byEmployeeAllDates(employeeId), {
        params: {
          month: month,
          year: year
        }
      ...
      if (response.data.success) {
        ...
        const leaveCount = formattedData.filter(record =>
          record.status.toLowerCase().includes('leave') || record.status.toLowerCase() === 'leave'
        ).length;
        const weekendCount = formattedData.filter(record =>
          record.status.toLowerCase().includes('weekend') || record.status.toLowerCase() === 'weekend'
        ).length;

...
//   const [fullData, setFullData] = useState([]); // This will now serve as "total loaded across all pages" or just removed if not needed.
//   const [filteredData, setFilteredData] = useState([]); // This will now be the data for the current page.
//   const [loading, setLoading] = useState(true);
//   const [error, setError] = useState(null);
//   const [exportData, setExportData] = useState([]);
//   const [isExporting, setIsExporting] = useState(false);
...
//         note: fu.notes || "",
//         date: fu.date
//           ? `${String(new Date(fu.date).getDate()).padStart(2, "0")}/${String(new Date(fu.date).getMonth() + 1).padStart(2, "0")
//           }/${new Date(fu.date).getFullYear()}`
//           : "",
//         createdBy: fu.createdBy || { fullName: "Unknown", role: "" },
//         createdAt: fu.createdAt
//           ? `${new Date(fu.createdAt).getDate().toString().padStart(2, "0")}/${(new Date(fu.createdAt).getMonth() + 1).toString().padStart(2, "0")
//           }/${new Date(fu.createdAt).getFullYear()}`
//           : "",
//       }));
...
//         followUps: followUps,
//         createdAt: doctor.createdAt,
//         createdAtTimestamp: doctor.createdAt ? new Date(doctor.createdAt).getTime() : 0,
...
//         params.specialization = searchBySpecialty;
//       }

//       if (searchBySalesman) {
//         params.salesmanName = searchBySalesman;
//       }

//       if (sortByDoctorStatus && sortByDoctorStatus !== "all") {
//         params.status = sortByDoctorStatus;
//       }

//       if (sortByDateFrom) {
//         params.dateFrom = sortByDateFrom;
//       }

//       if (sortByDateTo) {
//         params.dateTo = sortByDateTo;
//       }
...
//       const mappedData = mapDoctorData(doctorsData);
//       setFilteredData(mappedData);
//       setTotalDocs(pagination.total || mappedData.length);

//       // For old stats compatibility
//       setFullData(mappedData);
//     } catch (err) {
//       console.error("Error fetching doctors:", err);
//       setError("Failed to load doctors. Please try again.");
...
//       setLoading(false);
//     }
//   }, [currentPage, pageSize, searchByName, sortByStatus, sortByMembership, searchByCity, searchBySpecialty, searchBySalesman, sortByDoctorStatus, sortByDateFrom, sortByDateTo]);
...
//   // Enhanced filter function
//   // Enhanced filter function (Now just resets page and triggers fetch)
//   const applyFilters = useCallback(() => {
//     setCurrentPage(1);
//     fetchDoctors(1, pageSize);
//   }, [fetchDoctors, pageSize]);
...
//       setIsInitialLoad(false);
//     }
//   }, [fetchDoctors, isInitialLoad, pageSize]);

//   // Removed the useEffect that was auto-calling applyFilters
//   // It was causing pagination to reset to page 1 on every page change
//   // applyFilters is now only used for the "Apply Filters" button
...
//       header: "Membership",
//       render: (row) => (
//         <span className={`px-3 py-1.5 rounded-md text-xs font-medium ${row.typeOfMembership === "Hospital" ? "bg-purple-100 text-purple-800 border border-purple-200" :
//           row.typeOfMembership === "Individual" ? "bg-green-100 text-green-800 border border-green-200" :
//             row.typeOfMembership === "Hospital + Individual" ? "bg-indigo-100 text-indigo-800 border border-indigo-200" :
//               "bg-gray-100 text-gray-800 border border-gray-200"
//           }`}>
...
//             <div className="flex items-center justify-between">
//               <div>
//                 <p className="text-sm text-gray-600">Page Follow-ups</p>
//                 <p className="text-2xl font-bold text-gray-800">
//                   {filteredData.reduce((sum, item) => sum + (item.followUps?.length || 0), 0)}
//                 </p>
//               </div>
//               <div className="p-3 bg-green-50 rounded-lg">
//                 <FiMessageSquare className="text-green-600" size={24} />
...
//             <div className="flex flex-col sm:flex-row justify-between items-center mt-8 pt-6 border-t">
//               <div className="text-sm text-gray-600 mb-4 sm:mb-0">
//                 Showing <span className="font-semibold text-teal-600">{(currentPage - 1) * pageSize + 1}</span> to{" "}
//                 <span className="font-semibold text-teal-600">{Math.min(currentPage * pageSize, totalDocs)}</span> of{" "}
//                 <span className="font-semibold text-teal-600">{totalDocs}</span> doctors
//               </div>
//               <div className="flex gap-3">
//                 <button
//                   onClick={handleResetFilters}

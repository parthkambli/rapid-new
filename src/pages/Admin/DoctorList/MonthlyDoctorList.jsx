...
const MonthlyDoctorList = () => {
    ...
    const fetchMonthlyDoctors = async () => {
        try {
            setLoading(true);
            const response = await apiClient.get(apiEndpoints.adminDashboard.monthlyDoctors);
            if (response.data.success) {
                setDoctors(response.data.data || []);
            }
        } catch (err) {
            console.error("Error fetching monthly doctors:", err);
            setError("Failed to load monthly doctors");
        } finally {
            setLoading(false);
        }
    };
        ...
        return (
            <div className="p-6 bg-gray-50 min-h-screen flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="mt-4 text-gray-600">Loading monthly doctors...</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen">
            <div className="mb-6 flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-gray-800">Monthly Doctors</h1>
                    <p className="text-sm text-gray-600 mt-1">Total: {doctors.length} doctors added this month</p>
                </div>
                <button
                    onClick={() => navigate('/admin/dashboard')}
                            ...
                            {currentDoctors.length > 0 ? currentDoctors.map((doctor, index) => (
                                ...
                                <tr>
                                    <td colSpan="8" className="px-4 py-8 text-center text-gray-500">
                                        No doctors found for this month
                                    </td>
                                </tr>
                            )}
                        </tbody>
...
export default MonthlyDoctorList;

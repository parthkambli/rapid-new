...
const SalesmanDoctorList = () => {
    ...
    useEffect(() => {
        fetchSalesmanDoctors();
    }, []);

    const fetchSalesmanDoctors = async () => {
        try {
            setLoading(true);
            const response = await apiClient.get(apiEndpoints.adminDashboard.salesmanDoctors);
            if (response.data.success) {
                setDoctors(response.data.data || []);
            }
        } catch (err) {
            console.error("Error fetching salesman doctors:", err);
            setError("Failed to load salesman doctors");
        } finally {
            setLoading(false);
        }
    };
        ...
        return (
            <div className="p-6 bg-gray-50 min-h-screen flex items-center justify-center">
                <div className="text-center">
                    ...
                    <p className="mt-4 text-gray-600">Loading salesman doctors...</p>
                </div>
            </div>
        );
    }
    ...
    return (
        <div className="p-6 bg-gray-50 min-h-screen">
            <div className="mb-6 flex items-center justify-between">
                <div>
                    <h1 className="text-2xl font-bold text-gray-800">Doctors Added By Salesman</h1>
                    <p className="text-sm text-gray-600 mt-1">Total: {doctors.length} doctors added by salesmen</p>
                </div>
                <button
                    onClick={() => navigate('/admin/dashboard')}
                    ...
                    <table className="min-w-full divide-y divide-gray-200">
                        <thead className="bg-gray-50">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase">Added By (Salesman)</th>
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {currentDoctors.length > 0 ? currentDoctors.map((doctor, index) => (
                                <tr key={doctor._id || index} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-4 py-3 text-sm">
                                        <div>
                                            <div className="font-medium text-green-700">{doctor.createdBy?.fullName || 'N/A'}</div>
                                ...
                                <tr>
                                    <td colSpan="8" className="px-4 py-8 text-center text-gray-500">
                                        No doctors found added by salesmen
                                    </td>
                                </tr>
                            )}
                        </tbody>
...
export default SalesmanDoctorList;

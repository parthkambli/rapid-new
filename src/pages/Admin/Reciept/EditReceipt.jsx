...
//   const [loading, setLoading] = useState(true); // Start with true for proper loader
...
//   // Auto-select bank when banks are loaded and cashBankAc exists
//   useEffect(() => {
//     if (!loading && banks.length > 0 && formData.cashBankAc) {
//       // Ensure the bank dropdown is properly selected
//       // The key prop on the select element should handle this
//     }
//   }, [banks, formData.cashBankAc, loading]);

//   // Auto-select doctor when doctors are loaded and customer entityId exists
//   useEffect(() => {
//     if (!loading && doctors.length > 0 && formData.customer.entityId) {
//       // Ensure the doctor dropdown is properly selected
//       // The key prop on the select element should handle this
//     }
//   }, [doctors, formData.customer.entityId, loading]);

//   // Effect to ensure all form data is properly set after all data is loaded
//   useEffect(() => {
//     if (!loading && banks.length > 0 && doctors.length > 0 && formData.customer.entityId) {
//       // All data is loaded, ensure proper selections
//     }
//   }, [loading, banks, doctors, formData.customer.entityId]);
...
//   //       // Set tab
//   //       if (data.debitType || data.debitOn || data.debitDate || data.gst || data.monthlyPremium) {
//   //         setActiveTab("monthly");
//   //       } else if (data.nextInstallmentDate || data.installmentAmount) {
//   //         setActiveTab("yearly");
//   //       }

//   //       const customerInfo = data.customer || data.payer || {
//   //         type: "doctor",
//   //         entityId: "",
//   //         name: "",
//   //         contactPerson: "",
...
//   //         refCode: data.referenceNumber || data.bankDetails?.referenceNumber || "",
//   //         refDate: data.refDate || "",
//   //         paymentMethod: data.paymentMethod || "cash",
//   //         cashBankAc: data.bankDetails?.bankName || data.cashBankAc || "",
//   //         amount: data.amount || "",
//   //         chequeNo: data.bankDetails?.chequeNumber || data.chequeNo || "",
//   //         chequeDate: data.bankDetails?.chequeDate ? new Date(data.bankDetails.chequeDate).toISOString().split('T')[0] : "",
...
//       const data = response?.data || response; // Important: response.data ho sakta hai

//       if (!data) {
//         throw new Error("No receipt data received");
//       }

//       console.log("Received receipt data:", data); // Temporary – dekh kya aa raha hai

//       // Safe check – agar payer ya customer nahi hai to default
//       const payerInfo = data.customer || data.payer;

//       const customerInfo = payerInfo ? {
//         type: payerInfo.type || "doctor",
//         entityId: payerInfo.entityId || "",
//         name: payerInfo.name || "",
//         contactPerson: payerInfo.contactPerson || payerInfo.name || "",
//         phoneNumber: payerInfo.phoneNumber || "",
//         email: payerInfo.email || "",
//         address: payerInfo.address || { address: "", city: "", state: "", pinCode: "", country: "India" }
//       } : {
//         type: "doctor",
//         entityId: "",
//         name: "",
...
//         receiptNumber: data.receiptNumber || "",
//         receiptDate: data.receiptDate ? new Date(data.receiptDate).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
//         refCode: data.referenceNumber || (data.paymentAgainst?.type === 'bill' ? data.paymentAgainst.referenceId : "") || "",
//         refDate: data.refDate || "",
//         paymentMethod: data.paymentMethod || "cash",
...
//       // Set bill details for balance calculation - we need to fetch bill details separately
//       if (loadedFormData.refCode) {
//         try {
//           // First try to get the detailed bill information from receipt endpoints (more comprehensive)
//           let billResponse;
//           try {
//             billResponse = await apiHelpers.getById(apiEndpoints.receipts.billDetails, loadedFormData.refCode);
//           } catch (receiptError) {
//             // If receipt endpoint fails, fall back to sales bill endpoint
//             billResponse = await apiHelpers.getList(apiEndpoints.salesBills.getDoctorByBillNumber(loadedFormData.refCode));
//           }

//           const billResult = billResponse?.data || billResponse;

//           if (billResult) {
//             // Check if we got data from receipt/bill-details endpoint (has more fields)
//             const hasBillDetails = billResult.billId || billResult.billNumber; // This means we got detailed bill info
...
//               isInstallment: bill.isInstallment || false,
//               nextInstallmentDueDate: bill.nextInstallmentDueDate || null,
//               lastPaymentDate: bill.lastPaymentDate || null,
//               membership: bill.membershipTracking || null
//             });
//           } else {
//             // Set default bill details if no bill found
//             setBillDetails({
//               billId: null,
//               totalAmount: 0,
//               totalPaid: 0,
...
//           // Set default bill details if fetching fails
//           setBillDetails({
//             billId: null,
//             totalAmount: 0,
//             totalPaid: 0,
...
//         }
//       } else {
//         // Set default bill details if no ref code
//         setBillDetails({
//           billId: null,
//           totalAmount: 0,
//           totalPaid: 0,
...
//       // Set active tab based on receipt data
//       // Check if it's monthly type (has monthly-specific fields)
//       if (loadedFormData.debitType || loadedFormData.debitOn || loadedFormData.debitDate || loadedFormData.gst || loadedFormData.monthlyPremium) {
//         setActiveTab("monthly");
//       }
//       // Check if it's yearly type (has yearly-specific fields)
//       else if (loadedFormData.nextInstallmentDate || loadedFormData.installmentAmt || loadedFormData.paymentType === "installment") {
//         setActiveTab("yearly");
//       }

//       // First payment check only if entityId hai
//       if (customerInfo.entityId) {
//         try {
//           const res = await apiHelpers.getList(apiEndpoints.receipts.list, {
//             'payer.entityId': customerInfo.entityId,
//             'payer.type': 'doctor',
//             limit: 1
//           });
//           setIsFirstPayment(!(res?.data || res || []).length);
//         } catch (err) {
//           setIsFirstPayment(true);
//         }
//       }
...
//           const tab = tabType.includes('monthly') ? 'monthly' : 'yearly';
//           setActiveTab(tab);
//         }
//       }
...
//         const tabType = bill.membershipType.toLowerCase();
//         if (tabType.includes('monthly') || tabType.includes('year') || tabType === 'yearly') {
//           const newTab = tabType.includes('monthly') ? 'monthly' : 'yearly';
//           setActiveTab(newTab);
//         }
//       }
...
//         limit: 1
//       });
//       setIsFirstPayment(!(response?.data || response || []).length);
//     } catch (error) {
//       setIsFirstPayment(true);
//     }

//     // Fetch bill details for this doctor to get balance information
//     try {
//       const billResponse = await apiHelpers.getList(apiEndpoints.salesBills.getDoctorByBillNumber(latestBillNumber));
//       const billResult = billResponse?.data || billResponse;
...
//             address: {
//               address: selectedDoctor.address?.full || `${selectedDoctor.hospitalAddress?.address || ""}, ${selectedDoctor.hospitalAddress?.taluka || ""}, ${selectedDoctor.hospitalAddress?.city || ""}`.replace(/^,+/g, '').trim(),
...
//         });
//       } else {
//         // If no bill found with the latest bill number, just update customer info
//         setFormData(prev => ({
//           ...prev,
//           refCode: latestBillNumber || prev.refCode,
//           customer: {
//             ...prev.customer,
//             entityId: selectedDoctor._id,
//             name: displayName,
...
//       }
//     } catch (error) {
//       console.error("Error fetching bill details for doctor:", error);
//       // Fallback to just updating customer info if bill details fetch fails
//       setFormData(prev => ({
//         ...prev,
//         refCode: latestBillNumber || prev.refCode,
//         customer: {
...
//   //       let authObj;
//   //       try { authObj = JSON.parse(authString); } catch (e) { authObj = authString; }
...
//       if (!authString) {
//         console.warn("auth key not found in persist:root");
//         return null;
//       }
...
//       } catch (e) {
//         authObj = authString; // Agar already parsed hai
//       }

//       const user = authObj?.user || authObj;

//       if (!user || !user._id) {
//         console.warn("User or user._id not found:", user);
//         return null;
//       }

//       // HAMESHA _id use karo – yeh MongoDB ObjectId hai
//       const correctId = user._id;

//       // Extra safety: check if it's a valid 24-char hex string
//       if (typeof correctId !== 'string' || correctId.length !== 24) {
//         console.error("Invalid MongoDB _id format:", correctId);
//         return null;
//       }

//       console.log("Using correct user _id:", correctId); // → 6905ea97c1957ee4b383d699
//       return correctId;
...
//         receiptDate: formData.receiptDate,  // Use receiptDate instead of paymentDate
//         payer: {
//           type: "doctor",
...
//         paymentDate: formData.chequeDate || formData.receiptDate, // Keep for backward compatibility if needed
//         status: "received",
...
//             chequeDate: formData.chequeDate, // Use the actual stored date
...
//   const handleCancel = () => {
//     if (window.confirm("Cancel editing? All changes will be lost.")) {
//       navigate('/admin/receipt-list');
//     }
//   };
...
//           <div className="inline-block animate-spin rounded-full h-16 w-16 border-t-4 border-b-4 border-[#398C89]"></div>
//           <p className="mt-8 text-2xl font-semibold text-gray-800">Loading Receipt...</p>
//           <p className="mt-2 text-lg text-gray-600">Fetching doctor, bank and payment details</p>
//         </div>
//       </div>
//     );
//   }
...
//           </label>
//           <input type="text" value={formData.refCode} onChange={(e) => {
//             const val = e.target.value.toUpperCase();
//             handleInputChange('refCode', val);
//             if (billSearchTimeout.current) clearTimeout(billSearchTimeout.current);
//             billSearchTimeout.current = setTimeout(() => fetchDoctorByBillNumber(val), 800);
//           }} className="mt-1 p-2 w-full border-2 border-[#398C89] rounded-md font-semibold focus:ring-2 focus:ring-[#398C89]/30" placeholder="Type Bill No. → Auto-fill Doctor" />
//         </div>
//         <div>
...
//           <select
//             key={doctors.length > 0 ? formData.customer.entityId : 'loading'} // Use loading key until doctors are loaded
...
//                 {doctor.fullName}{doctor.hospitalName && ` - ${doctor.hospitalName}`}{doctor.membershipId && ` (${doctor.membershipId})`}{doctor.latestSalesBill && ` | Last Bill: ${doctor.latestSalesBill.billNumber}`}
//               </option>
//             ))}
//           </select>
//         </div>
...
//               <p className="mt-2 text-sm text-blue-700">This is the first payment for this customer. Special onboarding procedures may apply.</p>
//             </div>
//           </div>
//         </div>
//       )}
...
//           <input type="text" value={formData.customer.phoneNumber} onChange={(e) => handleInputChange('customer.phoneNumber', e.target.value)} className="mt-1 p-2 w-full border border-gray-300 rounded-md bg-gray-100" />
//         </div>
//         <div>
//           <label className="block text-sm font-medium text-gray-700">Email</label>
//           <input type="email" value={formData.customer.email} onChange={(e) => handleInputChange('customer.email', e.target.value)} className="mt-1 p-2 w-full border border-gray-300 rounded-md bg-gray-100" />
//         </div>
//       </div>
...
//           <input type="text" value={formData.customer.address.address} onChange={(e) => handleInputChange('customer.address.address', e.target.value)} className="mt-1 p-2 w-full border border-gray-300 rounded-md bg-gray-100" />
//         </div>
//         <div>
//           <label className="block text-sm font-medium text-gray-700">City</label>
//           <input type="text" value={formData.customer.address.city} onChange={(e) => handleInputChange('customer.address.city', e.target.value)} className="mt-1 p-2 w-full border border-gray-300 rounded-md bg-gray-100" />
//         </div>
//         <div>
//           <label className="block text-sm font-medium text-gray-700">State</label>
//           <input type="text" value={formData.customer.address.state} onChange={(e) => handleInputChange('customer.address.state', e.target.value)} className="mt-1 p-2 w-full border border-gray-300 rounded-md bg-gray-100" />
//         </div>
//         <div>
//           <label className="block text-sm font-medium text-gray-700">PIN Code</label>
//           <input type="text" value={formData.customer.address.pinCode} onChange={(e) => handleInputChange('customer.address.pinCode', e.target.value)} className="mt-1 p-2 w-full border border-gray-300 rounded-md bg-gray-100" />
//         </div>
//       </div>
...
//           <input type="number" value={formData.amount} onChange={(e) => handleInputChange('amount', e.target.value)} className="mt-1 p-2 w-full border border-gray-300 rounded-md bg-gray-100" required />
//         </div>
...
//               <input type="text" value={formData.chequeNo} onChange={(e) => handleInputChange('chequeNo', e.target.value)} className="mt-1 p-2 w-full border border-gray-300 rounded-md bg-gray-100" />
//             </div>
//             <div>
...
//             <select key={banks.length > 0 ? formData.cashBankAc : 'loading'} value={formData.cashBankAc} onChange={(e) => {
...
//                   {bank.bankName} - {bank.accountNumber} ({bank.ifscCode || "No IFSC"})
//                 </option>
//               ))}
//             </select>
//           </div>
...
//             <input type="text" value={formData.drawnOnBank} onChange={(e) => handleInputChange('drawnOnBank', e.target.value)} className="mt-1 p-2 w-full border border-gray-300 rounded-md bg-gray-100" placeholder="Enter bank name" />
//           </div>
//         </div>
//       )}
...
//                 <input type="number" value={formData.installmentAmt} onChange={(e) => handleInputChange('installmentAmt', e.target.value)} className="mt-1 p-2 w-full border border-gray-300 rounded-md bg-gray-100" />
//               </div>
//               <div>
...
//               <input type="text" value={formData.debitOn} onChange={(e) => handleInputChange('debitOn', e.target.value)} className="mt-1 p-2 w-full border border-gray-300 rounded-md bg-white" />
//             </div>
//             <div>
//               <label className="block text-sm font-medium text-gray-700">Debit Date:</label>
//               <input type="date" value={formData.debitDate} onChange={(e) => handleInputChange('debitDate', e.target.value)} className="mt-1 p-2 w-full border border-gray-300 rounded-md bg-white" />
//             </div>
//             <div>
//               <label className="block text-sm font-medium text-gray-700">GST:</label>
//               <input type="text" value={formData.gst} onChange={(e) => handleInputChange('gst', e.target.value)} className="mt-1 p-2 w-full border border-gray-300 rounded-md bg-white" />
//             </div>
//             <div className="md:col-span-2">
//               <label className="block text-sm font-medium text-gray-700">Monthly Premium:</label>
//               <input type="number" value={formData.monthlyPremium} onChange={(e) => handleInputChange('monthlyPremium', e.target.value)} className="mt-1 p-2 w-full border border-gray-300 rounded-md bg-white" />
//             </div>
//           </div>
//         </div>
//       )}
...
const EditReceipt = () => {
  ...
  // Naya state dropdown force re-render ke liye
  ...
  // Force dropdown re-render jab sab data load ho jaye
  useEffect(() => {
    if (!loading && doctors.length > 0 && banks.length > 0) {
      if (formData.customer.entityId || formData.cashBankAc) {
        setDropdownKey(prev => prev + 1);
      }
    }
  }, [loading, doctors.length, banks.length, formData.customer.entityId, formData.cashBankAc]);
  ...
  const fetchReceiptData = async () => {
    try {
      ...
      const customerInfo = payerInfo ? {
        type: payerInfo.type || "doctor",
        entityId: payerInfo.entityId?._id || payerInfo.entityId || "",
        name: payerInfo.name || "",
        contactPerson: payerInfo.contactPerson || payerInfo.name || "",
        phoneNumber: payerInfo.phoneNumber || "",
        email: payerInfo.email || "",
      ...
      const loadedFormData = {
        receiptNumber: data.receiptNumber || "",
        receiptDate: data.receiptDate ? new Date(data.receiptDate).toISOString().split('T')[0] : new Date().toISOString().split('T')[0],
        refCode: data.referenceNumber || (data.paymentAgainst?.type === 'bill' ? data.paymentAgainst.referenceId?._id || data.paymentAgainst.referenceId : "") || "",
        refDate: data.refDate || "",
        paymentMethod: data.paymentMethod || "cash",
        // cashBankAc: data.cashBankAc || "",
        cashBankAc: data.cashBankAc?._id || data.cashBankAc || data.bankDetails?.bankId || "",
        amount: data.amount || "",
        // chequeNo: data.chequeNo || "",
        // chequeDate: data.chequeDate ? new Date(data.chequeDate).toISOString().split('T')[0] : "",
...
chequeNo: data.bankDetails?.chequeNumber || data.chequeNo || "",
chequeDate: data.bankDetails?.chequeDate 
  ? new Date(data.bankDetails.chequeDate).toISOString().split('T')[0] 
  : (data.chequeDate ? new Date(data.chequeDate).toISOString().split('T')[0] : ""),
      ...
      if (loadedFormData.refCode) {
        try {
          ...
          if (billResult) {
            const hasBillDetails = billResult.billId || billResult.billNumber;
            const bill = hasBillDetails ? billResult : billResult.bill;
  ...
  const fetchDoctorByBillNumber = async (billNumber) => {
    ...
    try {
      ...
      const hasBillDetails = result.billId || result.billNumber;
      let doctor, bill;
      let doctorId;
      ...
      if (matchedDoctor) {
        ...
        if (hasBillDetails) {
          doctor = result.client?.entityId && typeof result.client.entityId === 'object' ? result.client.entityId : {
            _id: doctorId,
            fullName: result.client?.fullName || result.client?.name || "Dr. Unknown",
            hospitalName: result.client?.hospitalName || "",
            phoneNumber: result.client?.phoneNumber || "",
      ...
      if (matchedDoctor) {
        addressObj = {
          address: matchedDoctor.address?.full || `${matchedDoctor.hospitalAddress?.address || ""}, ${matchedDoctor.hospitalAddress?.taluka || ""}, ${matchedDoctor.hospitalAddress?.city || ""}`.replace(/^,+/g, "").trim(),
          city: matchedDoctor.hospitalAddress?.city || matchedDoctor.address?.city || "",
          state: matchedDoctor.hospitalAddress?.state || matchedDoctor.address?.state || "",
          pinCode: matchedDoctor.hospitalAddress?.pinCode || matchedDoctor.address?.pinCode || "",
          country: "India",
    ...
    try {
      const receiptData = {
        ...
        ...(formData.refCode && {
          paymentAgainst: {
            type: 'bill',
            referenceId: billDetails.billId || formData.refCode
          }
        }),
        bankDetails: {
          ...(formData.paymentMethod !== 'cash' && {
  ...
  return (
    <div className="max-w-[79vw] mx-auto p-6 bg-white rounded-lg shadow-lg">
      ...
      {formData.refCode && billDetails.membership && (
        <div className="mt-6 space-y-3">
          <h3 className="text-md font-semibold text-gray-800 flex items-center">📊 Membership Plan Details</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div><span className="text-gray-600">Type:</span><p className="font-medium text-gray-900">{billDetails.membership.type}</p></div>
            <div><span className="text-gray-600">Duration:</span><p className="font-medium text-gray-900">{billDetails.membership.planYears} Year{billDetails.membership.planYears > 1 ? 's' : ''} ({billDetails.membership.planTotalPeriods} {billDetails.membership.periodType})</p></div>
            <div><span className="text-gray-600">Rate:</span><p className="font-medium text-gray-900">₹{billDetails.membership.monthlyRate.toLocaleString('en-IN')} / {billDetails.membership.periodType === 'months' ? 'month' : 'year'}</p></div>
            <div><span className="text-gray-600">Expected Total:</span><p className="font-medium text-teal-700">₹{billDetails.membership.expectedTotalAmount.toLocaleString('en-IN')}</p></div>
          </div>
          <div className="pt-2">
            <div className="flex justify-between items-center text-sm mb-1">
              <span className="text-gray-600">Progress:</span>
              <span className={`font-bold ${billDetails.membership.isPlanCompleted ? 'text-green-600' : 'text-orange-600'}`}>
                {billDetails.membership.periodsPaid} / {billDetails.membership.planTotalPeriods} ({billDetails.membership.progressPercentage}%)
              </span>
            </div>
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div className={`h-full rounded-full transition-all ${billDetails.membership.isPlanCompleted ? 'bg-green-500' : 'bg-teal-500'}`} style={{ width: `${billDetails.membership.progressPercentage}%` }} />
            </div>
            <p className="text-sm text-gray-600 mt-1">
              Remaining: <strong>{billDetails.membership.remainingPeriods} {billDetails.membership.periodType}</strong>
              {billDetails.membership.isPlanCompleted && " ✅ Completed"}
      ...
      {formData.refCode && billDetails.isInstallment && (
        <div className="mt-6 space-y-3 bg-blue-50 p-4 rounded-lg border border-blue-200">
          <h3 className="text-md font-semibold text-blue-800 flex items-center">💳 Installment Summary</h3>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
            <div><span className="text-blue-600">Installment Type:</span><p className="font-medium text-blue-900">Active</p></div>
            <div><span className="text-blue-600">Total Amount:</span><p className="font-medium text-blue-900">₹{billDetails.totalAmount?.toLocaleString('en-IN')}</p></div>
            <div><span className="text-blue-600">Paid So Far:</span><p className="font-medium text-blue-900">₹{billDetails.totalPaid?.toLocaleString('en-IN')}</p></div>
            <div><span className="text-blue-600">Remaining:</span><p className="font-medium text-red-700">₹{billDetails.outstandingAmount?.toLocaleString('en-IN')}</p></div>
          </div>
          {billDetails.totalPaid > 0 && (
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
              <div><span className="text-blue-600">Last Payment:</span><p className="font-medium text-blue-900">₹{billDetails.totalPaid?.toLocaleString('en-IN')}</p></div>
              <div><span className="text-blue-600">Last Payment Date:</span><p className="font-medium text-blue-900">{billDetails.lastPaymentDate ? new Date(billDetails.lastPaymentDate).toLocaleDateString('en-IN') : 'N/A'}</p></div>
              <div><span className="text-blue-600">Next Installment Due:</span><p className="font-medium text-orange-700">{billDetails.nextInstallmentDueDate ? new Date(billDetails.nextInstallmentDueDate).toLocaleDateString('en-IN') : 'Not Set'}</p></div>
            </div>
          )}
          <div className="pt-2">
            <div className="w-full bg-gray-200 rounded-full h-2">
              <div className="h-full rounded-full bg-blue-500 transition-all" style={{ width: `${((billDetails.totalPaid || 0) / (billDetails.totalAmount || 1)) * 100}%` }} />
            </div>
            <p className="text-sm text-blue-600 mt-1">
              Payment Progress: <strong>{Math.round(((billDetails.totalPaid || 0) / (billDetails.totalAmount || 1)) * 100)}%</strong>
            </p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        <div>
          <label className="block text-sm font-medium text-gray-700">Select Doctor/Hospital *</label>
          <select
            key={`doctor-${formData.customer.entityId}-${dropdownKey}`}
            value={formData.customer.entityId}
            onChange={(e) => handleDoctorSelect(e.target.value)}
            className="mt-1 p-2 w-full border border-gray-300 rounded-md bg-gray-100"
            required
        ...
        {formData.refCode && (
          <div className="space-y-2">
            <div className="grid grid-cols-2 gap-2">
              <div>
                <label className="block text-xs font-medium text-gray-500">Total Amount</label>
                <input type="number" readOnly value={billDetails.totalAmount} className="mt-1 p-2 w-full border border-gray-300 rounded-md bg-gray-100 cursor-not-allowed text-sm" />
              </div>
              <div>
                <label className="block text-xs font-medium text-gray-500">Paid Amount</label>
                <input type="number" readOnly value={billDetails.totalPaid} className="mt-1 p-2 w-full border border-gray-300 rounded-md bg-gray-100 cursor-not-allowed text-sm" />
              </div>
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700">Balance Left to Pay</label>
              <input type="number" readOnly value={billDetails.outstandingAmount} className="mt-1 p-2 w-full border-2 border-[#398C89] rounded-md bg-gray-100 font-semibold cursor-not-allowed" />
            </div>
          </div>
        )}
        {formData.paymentMethod === 'cheque' && (
      ...
      {formData.paymentMethod !== 'cash' && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
          <div>
            <label className="block text-sm font-medium text-gray-700">Our Bank Account *</label>
            <select
              key={`bank-${formData.cashBankAc}-${dropdownKey}`}

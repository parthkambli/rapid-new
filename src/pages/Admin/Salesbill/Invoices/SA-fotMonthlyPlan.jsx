// Updated pages/service-agreement/MonthlySA.jsx
...
import SectionBox from '../Invoices/serviceAgreement(SA)/SectionBox';
import InfoTable from '../Invoices/serviceAgreement(SA)/InfoTable';
import ParagraphBlock from '../Invoices/serviceAgreement(SA)/ParagraphBlock';
import WitnessPoints from '../Invoices/serviceAgreement(SA)/WitnessPoints';
import TermsConditions from '../Invoices/serviceAgreement(SA)/TermsConditionMonthly';
import ScopeOfServices from '../Invoices/serviceAgreement(SA)/ScopeOfservices';
import SignatureBlock from '../Invoices/serviceAgreement(SA)/SignatureBlock';

const MonthlySA = () => {
  const { id } = useParams();
  const { doctor, members, policies, salesBill, loading, error } = useServiceAgreementData('monthly', id);
  ...
  return (
    <>
      <div className="text-end max-w-[80vw] sm:mb-4 my-6 print:hidden">
        <button
          onClick={handlePrint}
          className="px-6 py-3 bg-blue-600 text-white font-semibold rounded-lg shadow-lg hover:bg-blue-700 transition-colors text-sm"
        >
          🖨️ Print SA
        </button>
        <button
      ...
      <div className="font-[Times New Roman] text-[12px] leading-[1.4] print:text-black bg-white">
        ...
        <div className="a4-container">
          ...
          <div className="break-inside-avoid print:break-inside-avoid">
            ...
            <div className="section-box print:break-inside-avoid">
              <SectionBox title="SERVICE AGREEMENT">
                <p className="mb-2 print:mb-2">This Agreement for providing professional services is made at <b>Kolhapur</b> on this day of <b>{agreementDate}</b></p>
                ...
                {doctor.hasSpouse && doctor.spouseInfo ? (
                  ...
                  <p className="mb-2 print:mb-2">
                    <b>{doctor.fullName} & {doctor.spouseInfo.fullName}</b> residing at {doctor.address}, {doctor.city}, {doctor.taluka}, {doctor.district}, PIN CODE - {doctor.pincode}, {doctor.state}, {doctor.country},  with medical council number <b>{doctor.medicalCouncilNumber} & {doctor.spouseInfo.medicalCouncilNumber}</b> Herein after referred as the doctor the one part.
                  </p>
                  ...
                  <p className="mb-2 print:mb-2">
                    ...
                    <b>{doctor.hospitalName}</b> having its registered hospital at  {doctor.address},{doctor.city}, {doctor.taluka}, {doctor.district},  {doctor.state}, {doctor.country}, PIN CODE - {doctor.pincode}, Herein after referred as the doctor the one part.
                  </p>
                  ...
                  <p className="mb-2 print:mb-2">
                    <b>{doctor.fullName}</b> (representing {doctor.hospitalName}) residing at {doctor.address}, {doctor.city},{doctor.taluka}, {doctor.district},  {doctor.state}, {doctor.country},  PIN CODE - {doctor.pincode},  with medical council number <b>{doctor.medicalCouncilNumber}</b> Herein after referred as the doctor the one part.
                  </p>
                ) : (
                  // ✅ Case 4: Individual doctor - show doctor's personal address
                  <p className="mb-2 print:mb-2">
                    <b>{doctor.fullName}</b> residing at {doctor.address},{doctor.city}, {doctor.taluka}, {doctor.district},  {doctor.state}, {doctor.country}, PIN CODE - {doctor.pincode},  with medical council number <b>{doctor.medicalCouncilNumber}</b> Herein after referred as the doctor the one part.
                  </p>
                )}
                  <p className="mb-2 text-center uppercase font-bold print:mb-2">AND</p>
          ...
          {/* PAGE 2 - POLICY DETAILS, PAYMENT DETAILS, UPCOMING PAYMENT SCHEDULE, NOTE, WHEREAS */}
          <div className="break-inside-avoid print:break-inside-avoid">
            <div className='print:pt-[10px]'>
              <InfoTable title="Policy Details" tableType="policy" data={policies} />
            </div>

            {/* Single row layout for Payment Details and Upcoming Payment Schedule */}
          ...
          <div className="break-inside-avoid print:break-inside-avoid">
            <ParagraphBlock>
              <p className="mb-2 font-bold uppercase print:mb-2">WHEREAS</p>
              {doctor.doctorType === 'hospital' ? (
              ...
              <p className="print:text-[12px]">The Doctor has approaches to the company and applied for the service package offered by the Company on a monthly basis (via NACH Auto Debit)/Annual and the Company has agreed to provide its services in accordance with the terms and conditions set forth herein.</p>
            </ParagraphBlock>
          </div>
          ...
          {/* PAGE 7 - SCOPE OF SERVICES + SIGNATURE (for monthly) */}
          <div className="break-before-page break-inside-avoid min-h-[140mm] flex flex-col justify-end print:break-before-page print:break-inside-avoid">
            <ScopeOfServices color="text-black" />
            <SignatureBlock
              doctorName={
                doctor.hasSpouse && doctor.spouseInfo

...
// const SalesEditDoctor = () => {
//   const { id } = useParams();
//   const navigate = useNavigate();

//   const [membershipType, setMembershipType] = useState("Individual");
//   const [doctors, setDoctors] = useState([{}]); // For future multi-doctor support
...
//       try {
//         setLoading(true);
//         // Use the with-spouse endpoint to get both main doctor and linked spouse if they exist
//         const res = await apiClient.get(`/doctors/${id}/with-spouse`);
...
//         // Handle both single doctor and spouse-linked doctors
//         let doc = res.data.data;
//         let linkedDoc = null;

//         // Check if this is a linked doctor response
//         if (res.data.data.mainDoctor) {
//           // This means we have a spouse-linked doctor
//           doc = res.data.data.mainDoctor;
//           linkedDoc = res.data.data.linkedDoctor;
//         }
...
//         // Handle spouse-linked doctors
//         if (linkedDoc) {
//           // Set both doctors in the array
//           setDoctors([
//             {
//               ...doc,
...
//             {
//               ...linkedDoc,
//               doctorName: linkedDoc.fullName || "",
//               specialty: linkedDoc.specialization?.[0] || "",
//               mobile: linkedDoc.phoneNumber || "",
//               whatsapp: linkedDoc.whatsappNumber || "",
//               qualification: linkedDoc.qualification || "",
...
//         // Set form data with main doctor's info
//         const filledData = {
...
//             ? new Date(doc.nextFollowUpDate).toISOString().split("T")[0]
//             : "",
...
//           hospitalAddress:
//             doc.hospitalAddress?.address ||
//             doc.contactDetails?.currentAddress?.address ||
//             "",
...
//           hospitalPan: doc.hospitalDetails?.hospitalPanNumber || "",
...
//         setInitialFormData(filledData); // Critical for PATCH comparison

//         if (doc.doctorType === "hospital_individual") {
//           setDoctors(prev => prev.length === 1 ? [...prev, {}] : prev);
//         }
//       } catch (err) {
...
//     try {
//       // Helper to add only changed fields
//       const addIfChanged = (key, current, initial, transform = (v) => v) => {
//         const curr = current?.toString().trim() || "";
//         const init = initial?.toString().trim() || "";

//         if (curr !== init) {
//           payload[key] = curr === "" ? null : transform(curr);
//         }
//       };

//       // Doctor Personal Info
//       addIfChanged("fullName", formData.doctorName, initialFormData.doctorName);
//       addIfChanged("email", formData.email, initialFormData.email);
//       addIfChanged("phoneNumber", formData.mobile, initialFormData.mobile);
//       addIfChanged("whatsappNumber", formData.whatsapp, initialFormData.whatsapp);
//       addIfChanged("qualification", formData.qualification, initialFormData.qualification);
//       addIfChanged("licenseNumber", formData.regLicenseNo, initialFormData.regLicenseNo);

//       if (formData.specialty?.trim() !== (initialFormData.specialty || "")) {
//         payload.specialization = formData.specialty?.trim()
//           ? [formData.specialty.trim()]
//           : [];
//       }

//       // Hospital Name
//       addIfChanged("hospitalName", formData.hospitalName, initialFormData.hospitalName);

//       // Address changed?
//       if (
//         formData.hospitalAddress?.trim() !== (initialFormData.hospitalAddress || "") ||
//         formData.hospitalPinCode?.trim() !== (initialFormData.hospitalPinCode || "")
//       ) {
//         payload.hospitalAddress = {
//           address: formData.hospitalAddress?.trim() || null,
//           pinCode: formData.hospitalPinCode?.trim() || null,
//         };
//       }

//       // Status & Follow-up
//       addIfChanged("doctorStatus", formData.status, initialFormData.status, (v) =>
//         v.toLowerCase()
//       );
//       addIfChanged("nextFollowUpDate", formData.followUpDate, initialFormData.followUpDate);
//       addIfChanged("remarks", formData.remarks, initialFormData.remarks);

//       // Hospital Details (deep comparison)
//       const hospitalFieldsChanged =
//         formData.hospitalType !== initialFormData.hospitalType ||
//         formData.numberOfBeds !== initialFormData.numberOfBeds ||
//         formData.yearOfEstablishment !== initialFormData.yearOfEstablishment ||
//         formData.ownershipType !== initialFormData.ownershipType ||
//         formData.medicalSuperintendentName !== initialFormData.medicalSuperintendentName ||
//         formData.adminOfficerName !== initialFormData.adminOfficerName ||
//         formData.directorContactNo !== initialFormData.directorContactNo ||
//         formData.hospitalPan !== initialFormData.hospitalPan ||
//         formData.contactNo !== initialFormData.contactNo ||
//         formData.hospitalWhastApp !== initialFormData.hospitalWhastApp ||
//         formData.hospitalEmail !== initialFormData.hospitalEmail;

//       if (hospitalFieldsChanged) {
//         payload.hospitalDetails = {
//           hospitalType: formData.hospitalType,
//           beds: parseInt(formData.numberOfBeds) || 0,
//           establishmentYear:
//             formData.yearOfEstablishment?.trim() || null,
//           ownershipType: formData.ownershipType,
//           director: {
//             name: formData.medicalSuperintendentName?.trim() || null,
//             contact: formData.directorContactNo?.trim() || null,
//             email: formData.hospitalEmail?.trim() || null,
//           },
//           admin: {
//             name: formData.adminOfficerName?.trim() || null,
//             contact: formData.directorContactNo?.trim() || null,
//             email: formData.hospitalEmail?.trim() || null,
//           },
//         };

//         if (formData.hospitalPan?.trim() !== (initialFormData.hospitalPan || "")) {
//           payload.hospitalDetails.hospitalPanNumber = formData.hospitalPan?.trim() || null;
//         }

//         // Contact details for hospital
//         payload.contactDetails = {
//           phoneNumber: formData.contactNo?.trim() || null,
//           whatsapp: formData.hospitalWhastApp?.trim() || null,
//           emailId: formData.hospitalEmail?.trim() || null,
//           currentAddress: {
//             address: formData.hospitalAddress?.trim() || null,
//             pinCode: formData.hospitalPinCode?.trim() || null,
//           }
...
//       const res = await apiClient.patch(`/doctors/${id}`, payload);
...
//     const shouldRender = index === 0 || (doctors[index] && Object.keys(doctors[index]).length > 0 && doctors[index].doctorName);
...
//           {doctors.length > 1 && index > 0 ? `Doctor ${index + 1} Details` : 'Doctor Details'}
//         </h2>
//         <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
//           <div><input type="text" placeholder="Enter Doctor Name" value={doctors[index]?.doctorName || (index === 0 ? formData.doctorName : "")} onChange={(e) => {
//             if (index === 0) handleChange(e, 'doctorName');
//             const updatedDoctors = [...doctors];
//             updatedDoctors[index] = { ...updatedDoctors[index], doctorName: e.target.value.toUpperCase() };
//             setDoctors(updatedDoctors);
//           }} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-teal-500 outline-none" /></div>
//           <div><input type="text" placeholder="Enter Specialty" value={doctors[index]?.specialty || (index === 0 ? formData.specialty : "")} onChange={(e) => {
//             if (index === 0) handleChange(e, 'specialty');
//             const updatedDoctors = [...doctors];
//             updatedDoctors[index] = { ...updatedDoctors[index], specialty: e.target.value.toUpperCase() };
//             setDoctors(updatedDoctors);
//           }} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-teal-500 outline-none" /></div>
//           <div><input type="text" placeholder="Enter Mobile No" value={doctors[index]?.mobile || (index === 0 ? formData.mobile : "")} onChange={(e) => {
//             if (index === 0) handleChange(e, 'mobile');
//             const updatedDoctors = [...doctors];
//             updatedDoctors[index] = { ...updatedDoctors[index], mobile: e.target.value };
//             setDoctors(updatedDoctors);
//           }} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-teal-500 outline-none" /></div>
//           <div><input type="text" placeholder="Enter WhatsApp No" value={doctors[index]?.whatsapp || (index === 0 ? formData.whatsapp : "")} onChange={(e) => {
//             if (index === 0) handleChange(e, 'whatsapp');
//             const updatedDoctors = [...doctors];
//             updatedDoctors[index] = { ...updatedDoctors[index], whatsapp: e.target.value };
//             setDoctors(updatedDoctors);
//           }} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-teal-500 outline-none" /></div>
//           <div><input type="text" placeholder="Qualification" value={doctors[index]?.qualification || (index === 0 ? formData.qualification : "")} onChange={(e) => {
//             if (index === 0) handleChange(e, 'qualification');
//             const updatedDoctors = [...doctors];
//             updatedDoctors[index] = { ...updatedDoctors[index], qualification: e.target.value.toUpperCase() };
//             setDoctors(updatedDoctors);
//           }} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-teal-500 outline-none" /></div>
//           <div><input type="email" placeholder="Enter Email ID" value={doctors[index]?.email || (index === 0 ? formData.email : "")} onChange={(e) => {
//             if (index === 0) handleChange(e, 'email');
//             const updatedDoctors = [...doctors];
//             updatedDoctors[index] = { ...updatedDoctors[index], email: e.target.value.toLowerCase() };
//             setDoctors(updatedDoctors);
//           }} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-teal-500 outline-none" /></div>
//         </div>
...
//       <h2 className="text-lg font-semibold text-gray-800 mb-4">Hospital / Clinic Details</h2>
//       <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
//         <div><input type="text" placeholder="Hospital / Clinic Name" value={formData.hospitalName} onChange={(e) => handleChange(e, "hospitalName")} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-teal-500 outline-none" /></div>
//         <div>
...
//         <div><input type="text" placeholder="Pin Code" value={formData.hospitalPinCode} onChange={(e) => handleChange(e, "hospitalPinCode")} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-teal-500 outline-none" /></div>
//       </div>
//     </div>
//   );
...
//     <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
//       <h2 className="text-lg font-semibold text-gray-800 mb-4">Complete Hospital Details</h2>
//       <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
//         <div><input type="text" placeholder="Hospital Name" value={formData.hospitalName} onChange={(e) => handleChange(e, "hospitalName")} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-teal-500 outline-none" /></div>
...
//         <div><input type="text" placeholder="Year of Establishment" value={formData.yearOfEstablishment} onChange={(e) => handleChange(e, "yearOfEstablishment")} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-teal-500 outline-none" /></div>
...
//         <div><input type="text" placeholder="Medical Superintendent Name" value={formData.medicalSuperintendentName} onChange={(e) => handleChange(e, "medicalSuperintendentName")} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-teal-500 outline-none" /></div>
//         <div><input type="text" placeholder="Admin Officer Name" value={formData.adminOfficerName} onChange={(e) => handleChange(e, "adminOfficerName")} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-teal-500 outline-none" /></div>
//         <div><input type="text" placeholder="Admin Contact No" value={formData.directorContactNo} onChange={(e) => handleChange(e, "directorContactNo")} className="w-full px-4 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-teal-500 outline-none" /></div>
//       </div>
//     </div>
//   );

//   if (loading) {
//     return <div className="p-10 text-center text-gray-600">Loading doctor details...</div>;
//   }

//   return (
//     <div className="p-6 bg-gray-50 min-h-screen font-lato">
//       <h1 className="text-3xl font-bold text-gray-800 mb-8">Edit Doctor / Hospital</h1>

//       <div className="bg-white rounded-lg shadow-sm p-6 mb-6">
//         <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
//           <div><input type="text" value={formData.mlId} readOnly className="w-full px-4 py-2 border border-gray-300 rounded-md bg-gray-100" placeholder="MLID" /></div>
...
//           <div><input type="text" value={membershipType} readOnly className="w-full px-4 py-2 border border-gray-300 rounded-md bg-teal-50 text-teal-700 font-semibold" /></div>
//         </div>
//       </div>
...
//           {renderHospitalDetailsForIndividual()}
//         </>
//       )}
...
//           disabled={saving}
//           className={`px-8 py-3 text-white font-medium rounded-md transition ${saving
...
//           className="px-8 py-3 bg-orange-500 text-white font-medium rounded-md hover:bg-orange-600 transition"
//         >
//           Cancel
//         </button>
//       </div>
...
// export default SalesEditDoctor;
...
//           if (doctorData.mainDoctor?.doctorType === "hospital") {
//             membershipTypeValue = "Hospital";
//           } else if (doctorData.mainDoctor?.doctorType === "hospital_individual") {
...
//             hospitalPan: "", // This might need to come from hospitalDetails
...
//           // Set main form data
//           setMainFormData({
//             mlId: doctorData.mainDoctor?.membershipId || "",
//             date: doctorData.mainDoctor?.createdAt 
//               ? new Date(doctorData.mainDoctor.createdAt).toISOString().split('T')[0]
//               : new Date().toISOString().split('T')[0],
//             status: doctorData.mainDoctor?.doctorStatus || "cold",
//             followUpDate: doctorData.mainDoctor?.followUps?.[0]?.date 
//               ? new Date(doctorData.mainDoctor.followUps[0].date).toISOString().split('T')[0]
//               : "",
//             remarks: doctorData.mainDoctor?.remarks || "",
//             hospitalType: doctorData.mainDoctor?.hospitalDetails?.hospitalType || "Hospital",
//             numberOfBeds: doctorData.mainDoctor?.hospitalDetails?.beds || "",
//             yearOfEstablishment: doctorData.mainDoctor?.hospitalDetails?.establishmentYear || "",
//             ownershipType: doctorData.mainDoctor?.hospitalDetails?.ownershipType || "Private",
//             medicalSuperintendentName: doctorData.mainDoctor?.hospitalDetails?.director?.name || "",
//             adminOfficerName: doctorData.mainDoctor?.hospitalDetails?.admin?.name || "",
//             directorContactNo: doctorData.mainDoctor?.hospitalDetails?.director?.contact || "",
//             doctorType: doctorData.mainDoctor?.doctorType || "individual",
//             // Hospital fields
//             hospitalName: doctorData.mainDoctor?.hospitalName || "",
//             hospitalAddress: doctorData.mainDoctor?.hospitalAddress?.address || "",
//             hospitalPinCode: doctorData.mainDoctor?.hospitalAddress?.pinCode || "",
//             regLicenseNo: doctorData.mainDoctor?.licenseNumber || "",
//             hospitalPan: "", // Might need mapping
//             contactNo: doctorData.mainDoctor?.contactDetails?.phoneNumber || "",
//             hospitalWhastApp: doctorData.mainDoctor?.contactDetails?.whatsapp || "",
//             hospitalEmail: doctorData.mainDoctor?.contactDetails?.emailId || "",
//           });
...
//       // Prepare update data for each doctor
...
//         // If this is an existing doctor (has _id), update it
//         // Otherwise, we need to fetch the existing doctor ID from the API response
//         // For simplicity, we'll update the first doctor with the given ID
//         if (i === 0) {
//           updatePromises.push(
//             apiClient.patch(apiEndpoints.doctors.update(id), doctorData)
//           );
//         }
//         // For additional doctors (spouses), we need to handle creation/updating
...
//       // Handle spouse linking if needed
//       if (doctors.length === 2 && linkSpouses && (membershipType === "Individual" || membershipType === "Hospital + Individual")) {
//         // This would require additional logic to link existing doctors
//         toast.info("Spouse linking feature requires additional implementation");
//       }
...
//       toast.error("Error updating doctor: " + (error.message || "Unknown error"));
//     } finally {
//       setSaving(false);
//     }
//   };
...
//       // Handle spouse linking - EXACTLY LIKE ADD FORM
//       if (doctors.length === 2 && linkSpouses && (membershipType === "Individual" || membershipType === "Hospital + Individual")) {
//         const doctor1Id = createdDoctors[0]?._id || originalDoctorIds[0];
//         const doctor2Id = createdDoctors[1]?._id || originalDoctorIds[1];

//         if (doctor1Id && doctor2Id) {
//           try {
//             // Update both doctors with spouse linking - Use PUT API for linking
...
//             toast.success("Doctors updated! (Spouse linking failed)");
//           }
//         }
//       } 
...
//     } catch (error) {
//       console.error("Error updating doctor:", error);
//       toast.error("Error updating doctor: " + (error.response?.data?.message || "Unknown error"));
//     } finally {
//       setSaving(false);
//     }
//   };
